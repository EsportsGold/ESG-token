...
contract('ESGToken', function(accounts) {
    ...
    it('11: verifies the balances after minting tokens', async () => {
        let token = await ESGToken.new();
        let decimals = await token.decimals.call();
        let supplyCap = 10000;
        await token.setTokenCapInUnits(supplyCap);
    ...
    it('17: throws if token burn is called (as asset contract will only be in place with value)', async () => {
        let token = await ESGToken.new();
        let decimals = await token.decimals.call();
        let supplyCap = 10000;
        await token.setTokenCapInUnits(supplyCap);
        await token.mint(accounts[0], 2250);            

        let burnAmount = 2250 * Math.pow(10, decimals);
